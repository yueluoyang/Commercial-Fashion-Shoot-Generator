...
1. Load and inspect the visible input images. If local paths are referenced but not visible in the conversation, use `view_image` first so the images are in context. Identify the first-batch/base product references and any early color-variant references. If the base color is ambiguous, ask which color should be used for the first four-image sample.
...
3. Recommend 2-3 indoor shoot styles using `references/indoor-set-styles.md`, unless the user already supplied a set style. If early color-variant references exist, make each recommended set compatible with the base color and all queued variant colors. If the user supplied a set style, validate it against the guide-based product analysis and all known colorways before accepting it.
4. Present the fused multimodal product analysis, fixed model proportion lock, queued color-variant note if applicable, and set options concisely, then ask the user to choose one option or provide their own.
...
- Queued color-variant note, if the user already supplied additional color references: list the known variant colors and state that they are deferred until first-batch approval.
- 2-3 indoor set style options: each with mood, props, light, why it fits the garment, and why the palette works for all known colorways.
...
- If early color-variant references were provided, state that they remain queued for after first-batch approval.
